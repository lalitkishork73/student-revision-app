 ...
 */
function parseQuizResponse(rawResponse: any): any[] {
  try {
    let content = "";
...
export async function generateQuizFromPDF(
  ...
  try {
    ...
    const chunks = await queryVectors(pdfId, embedding, 6);
...
Return the JSON object now.`;
    ...
    // Call OpenAI
    const rawResponse = await createChatCompletion(messages, {
      temperature: 0.3,
      maxTokens: 2000,
    });
 ...
 */
export async function getQuizQuestions(pdfId: string, userId: string) {
